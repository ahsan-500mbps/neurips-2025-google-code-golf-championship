...
def u9(patch):A=toindices(patch);return min(A for(A,B)in A),min(A for(B,A)in A)
def l9(patch):A=toindices(patch);return max(A for(A,B)in A),max(A for(B,A)in A)
def toindices(patch):
	A=patch
...
def d1(loc):A=loc;return{(A[0]-1,A[1]),(A[0]+1,A[1]),(A[0],A[1]-1),(A[0],A[1]+1)}
def objects(grid,u8,diagonal,without_bg):
	A=grid;H=mostcolor(A)if without_bg else None;I=[];D=set();L,M=len(A),len(A[0]);N=asindices(A);O=neighbors if diagonal else d1
	for B in N:
		if B in D:continue
		E=A[B[0]][B[1]]
		if E==H:continue
		...
		while F:
			...
			for C in F:
				G=A[C[0]][C[1]]
				if E==G if u8 else G!=H:J.add((G,C));D.add(C);K|={(A,B)for(A,B)in O(C)if 0<=A<L and 0<=B<M}
			F=K-D
		I.append(J)
	return I
def fg(grid):A=grid;B=mostcolor(A);return[{(A,(C,E))for(C,D)in enumerate(A)for(E,A)in enumerate(D)if A==B}for B in palette(A)-{B}]
...
def l2(a,b):return min(abs(A-C)+abs(B-D)for(A,B)in toindices(a)for(C,D)in toindices(b))
def palette(element):
	A=element
...
def p(I):
	...
	G=mostcolor(I);B=fg(I);H=objects(I,True,False,True)
	def J(area):
		D=next(iter(area))[0];A=[A for A in H if next(iter(A))[0]==D]
		if A:B=max((rightmost(A)-leftmost(A)+1 for A in A),default=0)
		else:B=0
		E={l2(C,B)for B in A for C in A};F={A for A in E if A!=0};C=min(F,default=0);G=C-1 if C>0 else-2;return-(2*B+G)
	K=sorted(B,key=J)
	def L(area):
		A=area
		def B(piece):return tuple(sorted(toindices(n1(piece))))
		C=vmirror(A),dmirror(A),cmirror(A),hmirror(A);return min(C,key=B)
	M=[L(A)for A in K];N=tuple(n1(A)for A in M);C=len(B);D=C if any(len(A)==1 for A in B)else C+1;O=tuple((A,A)for A in range(D));E=tuple(C for(A,B)in zip(N,O)for C in sorted(shift(A,B),key=lambda t:t[1]));F=2*D-1;A=canvas(G,(F,F))
