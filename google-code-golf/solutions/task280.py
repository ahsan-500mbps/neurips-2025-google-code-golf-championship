...
def _objects(grid):
	A=grid;I,J=len(A),len(A[0]);L=m1(A);F=[[False]*J for A in range(I)];M=[]
	for G in range(I):
		for H in range(J):
...
def t1(patch):
	...
	return list(A)
def b3(patch):A=t1(patch);B=[A for(A,B)in A];C=[A for(B,A)in A];return min(B),max(B),min(C),max(C)
def _height(patch):
	A=patch
	if not A:return 0
	B,C,D,D=b3(A);return C-B+1
def _width(patch):
	A=patch
	if not A:return 0
	B,B,C,D=b3(A);return D-C+1
def _uppermost(patch):B,A,A,A=b3(patch);return B
def _lowermost(patch):A,B,A,A=b3(patch);return B
def _leftmost(patch):A,A,B,A=b3(patch);return B
def _rightmost(patch):A,A,A,B=b3(patch);return B
def _vline(patch):
	A=t1(patch)
...
def _fill(grid,value,indices):
	...
	for(C,D)in t1(indices):
		if 0<=C<E and 0<=D<F:B[C][D]=value
	return B
def _underfill(grid,value,indices):
	A=grid;E,F=len(A),len(A[0]);G=m1(A);B=[list(A)for A in A]
	for(C,D)in t1(indices):
