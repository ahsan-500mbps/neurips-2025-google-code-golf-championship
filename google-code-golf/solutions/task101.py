...
def u8(pxs,f):return{(A,(B*f+D,C*f+E))for(A,(B,C))in pxs for D in range(f)for E in range(f)}
def o1(coords):
	A=coords
	if not A:return set()
...
def o(G,pattern):
	...
	D,E=len(G),len(G[0]);A=no(C);N=[A for(B,(A,C))in A];O=[A for(B,(C,A))in A];P,Q=max(N)+1,max(O)+1;F=set()
...
def solve_447fd412(I):
	...
	S=max(C,key=C.get);E=с8(I,S)
	if not E:return I
	E.sort(key=lambda c:len({A for(A,B)in c}),reverse=_A);L=E[0];T=no(L);U=Q(A for(A,B)in L);M=max(U.items(),key=lambda kv:(kv[1],-kv[0]))[0];F=[]
	for V in(1,2,3):
		G=u8(T,V);H={(A,B)for(A,B)in G if A!=M};B=set();A=set()
		if H:A={A for(B,A)in H};J=H|{(0,A)for A in o1(A)};B=o(I,J)
		if not B:
			K={(A,B)for(A,B)in G if A==M}
			if not K:continue
			A={A for(B,A)in K};J=K|{(0,A)for A in o1(A)};B=o(I,J)
		if not B:continue
