def p(g):
	...
	E,F=len(g),len(g[0])
	def A3(G,u8,da,wb):
		K=wb;from collections import Counter as U;Q=U(B for A in G for B in A).most_common(1)[0][0]if K else C;A=[[m]*F for A in range(E)];R=[];S=[(1,0),(-1,0),(0,1),(0,-1)]
		if da:S+=[(1,1),(1,-1),(-1,1),(-1,-1)]
		for B in range(E):
			for H in range(F):
				T=G[B][H]
				...
				while M:
					N,O=M.pop();P=G[N][O]
					if u8 and P!=V or K and P==Q:continue
					L.append((P,(N,O)))
					for(W,X)in S:
						I,J=N+W,O+X
	...
	def AA(le1,ib1,ic1):
		F=ib1;E=le1;from collections import Counter as X;G=p(E);Y=X(A for(A,B)in E if A not in(0,4));H=[A for A in G if A in F]
		if not H:return
		I,J=[],[];A=[];K=[]
		for B in H:
			L,Z,M,a=G[B];N,b,O,c=F[B];P=a-M+1;Q=Z-L+1;R=c-O+1;S=b-N+1
			if P<=0 or Q<=0 or R<=0 or S<=0:return
			d=max(1,q(R,P));e=max(1,q(S,Q));I.append(d);J.append(e);A.append((L,M,N,O));T=Y.get(B,0);f=ic1.get(B,0)
			if T>0:
				C=1
				while T*C*C<f:C+=1
				K.append(C)
