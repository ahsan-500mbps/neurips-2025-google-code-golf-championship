def s4(I):
	...
	f=D(I,u8=A,ignore_bg=A);g=sorted(f,key=lambda o:B([A for(B,A)in o]));G=F
	for C in g:
		if R(C,I):G=C;break
	if G is F:return I
	H=K([A for(B,A)in G]);S=D(I,u8=J,ignore_bg=A)
	if not S:return I
	h=sorted(S,key=lambda o:B([A for(B,A)in o]))[0];i,j=L(I,h);M=c(i);T=D(M,u8=J,ignore_bg=A)
	if not T:return I
	k=sorted(T,key=lambda o:B([A for(B,A)in o]))[0];l=D(M,u8=A,ignore_bg=A);N=F
	for C in sorted(l,key=lambda o:B([A for(B,A)in o])):
		if R(C,M):N=C;break
	if N is F:return I
	U=K([A for(B,A)in N]);m,n=H[0]-U[0],H[1]-U[1];E=P(I,Q(k,m,n));V=D(E,u8=J,ignore_bg=A)
	if not V:return E
	o=sorted(V,key=lambda o:B([A for(B,A)in o]))[0];p,j=L(E,o);W=d(p);X=D(W,u8=J,ignore_bg=A)
	if not X:return E
	q=sorted(X,key=lambda o:B([A for(B,A)in o]))[0];Y=D(W,u8=A,ignore_bg=A)
	if not Y:return E
	r=next(iter(G))[0];O=F
